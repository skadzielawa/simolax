...
(function($){
	...
	$.fn.simolax = function( options ) {
		...
		function setTransformation(el, img, ratio, isCentered) {
			if (typeof jQuery === "function" && el instanceof jQuery && img instanceof jQuery) {
        el = el[0];
				img = img[0];
	    }
			var rect = el.getBoundingClientRect();
			...
			if(isCentered) {
				...
				$(img).css('transform', 'translate3D(' + (-width/2) + 'px, ' + top * ratio + 'px, 0)' );
			} else {
				$(img).css('transform', 'translate3D( 0px, ' + top * ratio + 'px, 0)' );
			}
		}
